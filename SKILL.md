...
0. **工具链检测** — 检查XeLaTeX/latexmk/bibtex等工具是否存在，缺失则询问用户
1. **获取源文件** — 从arxiv网址/通用下载网址/本地目录获取LaTeX源码，放到 `/tmp/latex-translate-<id>/`
2. **项目结构分析** — 判断源码是否已分文件，决定串行/并行翻译策略
2.5. **基线编译与兼容性扫描** — 翻译前编译原始英文项目，扫描中文兼容性风险
3. **翻译执行** — 按翻译块规则翻译，已分文件时并行执行以加速
4. **中文编译与兼容性修复** — 用XeLaTeX编译翻译后的.tex，自动识别并修复兼容问题
4.5. **PDF视觉验收与日志验收** — 完整多遍编译后检查引用、排版、重叠等
5. **输出PDF** — 将验收通过的PDF复制到当前工作目录
...
### 4.4 Moving Arguments 中的 Fragile Macro 修复
...
如发现问题，回到 Step 4.2-4.4 对应的修复规则处理，然后重编译、重验收。
